...
public class RecipsController(IRecipsRepository recipsRepository) : ODataController
{
    private readonly IRecipsRepository _recipsRepository = recipsRepository;

    [EnableQuery]
    public IActionResult Get()
    {
        var getResult = _recipsRepository.GetItems();
    ...
    {
        var getResult = await _recipsRepository.GetItemAsync(k => k.Id == key);
    ...
    [HttpPost]
    public async Task<IActionResult> PostAsync([FromBody] RecipModel item)
    {
        var createResult = await _recipsRepository.CreateItemAsync(item);
    ...
    [HttpPatch]
    public async Task<IActionResult> PatchAsync([FromODataUri] Guid key, [FromBody] RecipModel item)
    {
        var updateResult = await _recipsRepository.UpdateItemAsync(k => k.Id == key, item);
    ...
    {
        var deleteResult = await _recipsRepository.DeleteItemAsync(k => k.Id == key);
